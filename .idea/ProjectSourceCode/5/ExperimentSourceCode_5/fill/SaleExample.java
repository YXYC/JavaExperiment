      ...
      if(receiveMoney==5){
         ...
         Thread.currentThread().getName()); 
          ...
          if(fiveNum>3){
              ...
              System.out.printf("\n%s给20元钱，找您三张5元，这是您的1块面包",Thread.currentThread().getName());
          }else{ 
              fiveNum=fiveNum-1;
              tenNum=tenNum-1;
              twentyNum=twentyNum+1;
              System.out.printf("\n%s给20元钱，找您一张5元和一张10元，这是您的1块面包",Thread.currentThread().getName());
          }
          ...
          if(fiveNum<2){
             tenNum=tenNum-1;
             twentyNum=twentyNum+1;
             System.out.printf("\n%s给20元钱，找您一张10元，这是您的2块面包",Thread.currentThread().getName());
          } 
      }else{ 
          fiveNum=fiveNum-2;
          twentyNum=twentyNum+1;
          System.out.printf("\n%s给20元钱，找您两张5元，这是您的2块面包",Thread.currentThread().getName());
          }
       notifyAll();
   }
}
class Breadshop implements Runnable{ //实现Runnable接口的类（面包店）。
   Thread zhao,qian,sun,li,zhou;//来面包店中买面包的线程。
   BreadSeller seller;     //面包店服务员
   Breadshop(){

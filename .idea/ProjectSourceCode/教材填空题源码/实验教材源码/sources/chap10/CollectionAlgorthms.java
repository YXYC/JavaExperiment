...
public class CollectionAlgorithms {
    public static void main(String[] args){
        ...
        for(int i=0;i<100;i++){
           __________________((int)(Math.random()*100));//添加元素
        }
       _____________________________;  //对集合进行排序
        ...
        System.out.println("Frequency of "+testNumber+": "+Collections.________________(list,testNumber));//testNumber出现的频率
        Set<Integer> sortedList=new HashSet<>();
        sortedList.addAll(list);
        System.out.println("Number of distinct elements: "+sortedList.size());
        list.clear();
        list.addAll(sortedList);
        ____________________________;  //搅乱集合元素顺序
